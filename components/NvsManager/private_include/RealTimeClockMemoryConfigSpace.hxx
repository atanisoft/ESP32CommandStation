...
namespace esp32cs
{
    ...
    public:
            ...
            node_config_t *config) : config_(config)
        {
            #define REGISTER_NUMBER(field, type, name) \
                LOG(ALWAYS, "[RealTimeClockMemCfg:%02x] register_numeric(%s): %d", SPACE, name, field.offset()); \
                register_numeric(field, numeric_reader<type>(field.offset()), \
                                numeric_writer<type>(field.offset()))
            #define REGISTER_STRING(field, name) \
                LOG(ALWAYS, "[RealTimeClockMemCfg:%02x] register_string(%s): %d", SPACE, name, field.offset()); \
                register_string(field, string_reader(field.offset()), \
                                string_writer(field.offset()))

            REGISTER_NUMBER(RealTimeClockConfigHolder.enabled(), uint8_t, "enabled");
    ...
    private:
        ...
        static constexpr uint8_t SPACE = CONFIG_OLCB_REALTIMECLOCK_MEMORY_SPACE_ID;
        template <typename T>
        ...
        numeric_reader(int offset)
        {
            ...
            {
                LOG(INFO, "[RealTimeClockMemCfg:%02x-RD] offs: %d", SPACE, offset);
                T value = (T)0;
                switch(offset)
                {
                    case RealTimeClockConfigHolder.enabled().offset():
                        ...
                        break;
                    default:
                        LOG_ERROR("[RealTimeClockMemCfg:%02x-RD] request for unrecognized offset:%d", SPACE, offset);
                }
                done->notify();
                return value;
            };
        ...
        numeric_writer(int offset)
        {
            ...
            {
                LOG(INFO, "[RealTimeClockMemCfg:%02x-WR] offs: %d", SPACE, offset);
                switch(offset)
                {
                    case RealTimeClockConfigHolder.enabled().offset():
                        ...
                        break;
                    default:
                        LOG_ERROR("[RealTimeClockMemCfg:%02x-WR] request for unrecognized offset:%d", SPACE, offset);
                }
                done->notify();
            };
        }
        ...
        string_reader(int offset)
        {
            ...
            {
                LOG(INFO, "[RealTimeClockMemCfg:%02x-RDSTR] offs: %d", SPACE, offset);
                switch (offset)
                {
                    case RealTimeClockConfigHolder.id().offset():
                        *value = node_id_to_string(config_->fastclock_id);
                        break;
                    default:
                        LOG_ERROR("[RealTimeClockMemCfg:%02x-WRSTR] request for unrecognized offset:%d", SPACE, offset);
                        *value = "";
                }
                done->notify();
            };
        ...
        string_writer(int offset)
        {
            return [this, offset](unsigned repeat, string value, BarrierNotifiable *done)
            {
                ...
                LOG(INFO, "[RealTimeClockMemCfg:%02x-WRSTR] offs: %d, value:%s", SPACE, offset, value.c_str());
                switch(offset)
                {
                    case RealTimeClockConfigHolder.id().offset():
                        ...
                        break;
                    default:
                        LOG_ERROR("[RealTimeClockMemCfg:%02x-WSTR] request for unrecognized offset:%d", SPACE, offset);
                }
                done->notify();
            };
        }
